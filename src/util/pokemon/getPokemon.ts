...
export default async ({ pokemon, learnsetBool = false, shinyBool = false, genData, emojis }) => {
    ...
    if (generation < pokemon.gen) {
        ...
        messageObject = { embeds: [pkmEmbed], components: [] as any };
    ...
    let afdSprite = `https://play.pokemonshowdown.com/sprites/afd/${pokemonSim.spriteid}.png`;
    ...
    if (learnsetBool && pokemonLearnset && pokemonAvailable) {
        ...
        levelMoves = levelMoves.sort((a: any, b: any) => a.split(levelMoveSplit)[1] - b.split(levelMoveSplit)[1]);
        // Prevo egg moves
        ...
        specialMoves = [...new Set(specialMoves)].filter((el: any) => !levelMovesNames.includes(el)).filter((el: any) => !tmMoves.includes(el)).filter((el: any) => !eggMoves.includes(el)).filter((el: any) => !tutorMoves.includes(el));
        specialMovesString = specialMoves.join(", ");
        transferMoves = [...new Set(transferMoves)].filter((el: any) => !levelMovesNames.includes(el)).filter((el: any) => !tmMoves.includes(el)).filter((el: any) => !eggMoves.includes(el)).filter((el: any) => !tutorMoves.includes(el)).filter((el: any) => !specialMoves.includes(el));
