...
export default async (offset: any) => {
    const trophies_per_page = 10;
    let trophy_list = await getTrophieslice(offset, trophies_per_page);
    const embed = new EmbedBuilder().setColor(globalVars.embedColor as [number, number, number]);
    ...
    return { embed: embed, components: [navigation_buttons.toJSON()] };
};