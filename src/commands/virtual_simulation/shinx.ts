...
export default async (interaction: any, messageFlags: any) => {
    ...
    switch (interaction.options.getSubcommand()) {
        ...
        case "battle":
            ...
            if (!trainer_answer) {
                return sendMessage({ interaction: interaction, content: `Battle cancelled, the challenge timed out.`, components: [] as any }).catch(e => { return null; });
            } else if (trainer_answer.customId === 'battleNo') {
                return sendMessage({ interaction: interaction, content: `Battle cancelled, ${trainers[1]} rejected the challenge.`, components: [] as any });
            };
            if (globalVars.battling.yes) return sendMessage({ interaction: interaction, content: `Theres already a battle going on.`, components: [] as any });
