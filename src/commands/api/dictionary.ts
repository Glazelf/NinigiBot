...
export default async (interaction: any, messageFlags: any) => {
    ...
    if (wordStatus[0].phonetics) {
        ...
        let wordPhoneticsArray = wordStatus[0].phonetics.filter((phonetic: any) => phonetic.text && phonetic.text.length > 0);
        let wordPhoneticsArrayAudio = wordPhoneticsArray.filter((phonetic: any) => phonetic.audio && phonetic.audio.length > 0); // Prefer entries with audio available
    ...
    await wordMeaning.definitions.forEach((definition: any) => {
