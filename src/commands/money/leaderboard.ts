    ...
    switch (scope) {
        case "global":
            ...
            let leaderboardStringGlobal = money_db.filter(user => interaction.client.users.cache.has(user.user_id))
                ...
                .slice(0, 10)
                .map((user: any, position: any) => `${position + 1}. ${(interaction.client.users.cache.get(user.user_id).username)}: ${formatNumber(Math.floor(user.money), interaction.locale)}${globalVars.currency}`)
                .join('\n');
            leaderboardEmbed
