...
export default async (client: any, messageReaction: any) => {
    try {
        let boardEmote = starboardEmote;
        // Check if message has reactions and if reaction is a star
        if (!messageReaction.count) messageReaction = await messageReaction.fetch().catch((e: any) => { return null; });
        ...
        let starboardChannel, starboard: any;
        ...
        if (messageReaction.count >= starLimit && !messageDB) {
            ...
            if (messageReaction.count >= starLimit * 2) starMessage.pin().catch((e: any) => {
                // console.log(e);
                return;
            });
            return;
        ...
        async function sendStarboardMessage(starboardMessage: any, targetMessage: any) {
            // @ts-expect-error TS(2339): Property 'StarboardMessages' does not exist on typ... Remove this comment to see the full error message
            await starboard.send(starboardMessage).then(async (m: any) => await serverApi.StarboardMessages.upsert({ channel_id: targetMessage.channel.id, message_id: targetMessage.id, starboard_channel_id: m.channel.id, starboard_message_id: m.id }));
        };
