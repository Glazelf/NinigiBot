...
const allPokemonByLength = Dex.species.all().filter(pokemon => pokemon.isNonstandard !== "CAP").sort((a: any, b: any) => a.name.length - b.name.length);
...
const allMegaStonesByLength = Dex.items.all().filter(item => item.megaStone && item.isNonstandard !== "CAP").sort((a: any, b: any) => {
...
let pokemonCardsAll = [];
fs.readdir("./submodules/pokemon-tcg-data/cards/en", (err: any, files: any) => {
    ...
    try {
        ...
        switch (interaction.type) {
            ...
            case InteractionType.ApplicationCommandAutocomplete:
                    ...
                    case "role":
                        switch (focusedOption.name) {
                            case "role":
                                ...
                                roleObject = roleObject.sort((r: any, r2: any) => r2.position - r.position);
                ...
                if (Object.keys(valuesByDate).length > 0) choices.sort((a: any, b: any) => valuesByDate[b.value] - valuesByDate[a.value]); // Sort from new to old
