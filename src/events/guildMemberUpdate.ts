        ...
        if (checkPermissions({ member: botMember, channel: log, permissions: [PermissionFlagsBits.SendMessages, PermissionFlagsBits.EmbedLinks] })) {
            ...
            switch (updateCase) {
                ...
                case "guildBanner":
                    ...
                    break;
                case "rolesUpdate":
                    let rolesSorted = [...oldMember.roles.cache.values()].filter(element => element.name !== "@everyone").sort((r: any, r2: any) => r2.position - r.position);
                    let newRolesSorted = [...newMember.roles.cache.values()].filter(element => element.name !== "@everyone").sort((r: any, r2: any) => r2.position - r.position);
