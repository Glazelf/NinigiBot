...
const { Shinx, EventTrophy, User } = require("../dbObjects/userdata.model")(
  userdata,
  Sequelize.DataTypes,
);
const { shinxQuotes } = require("../dbObjects/serverdata.model")(
  serverdata,
  Sequelize.DataTypes,
);
...
module.exports = {
  ...
  async getRandomShinx(amount, exclude, guild) {
    const results = await Shinx.findAll({
      attributes: ["user_id", "shiny", "user_male"],
      where: {
        user_id: { [Op.ne]: exclude, [Op.in]: [...guild.members.cache.keys()] },
      },
