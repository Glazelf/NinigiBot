...
const { User, ShopTrophy, EventTrophy } =
...
module.exports = {
  ...
  async getTrophieslice(offset, trophies_per_page) {
    let EventTrophies = await EventTrophy.findAll({
      attributes: ["trophy_id", "icon"],
    });
    let shoptrophies = await ShopTrophy.findAll({
      attributes: ["trophy_id", "icon"],
    });
    ...
    return {
      slice: trophies.slice(offset, offset + trophies_per_page),
      buttons: answer,
    };
  },
  ...
  async getBuyableShopTrophies(user_id) {
    ...
    today_trophies = today_trophies.filter(
      (trophy) => !(trophy in user_trophies_list),
    );
    return today_trophies;
  },
  ...
  async buyShopTrophy(user_id, trophy_id) {
    ...
    let trophy = trophies.find(
      (elem) => elem.trophy_id.toLowerCase() == trophy_id_t,
    );
