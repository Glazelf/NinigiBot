module.exports = async ({
  ...
  try {
    ...
    const typechart =
      require("../../node_modules/pokemon-showdown/dist/data/typechart.js").TypeChart;
    const learnsets =
    ...
    let typeString = getTypeEmotes({
      type1: type1,
      type2: type2,
      emotes: emotesAllowed,
    });
    ...
    for (let [typeName, matchup] of Object.entries(type1)) {
      ...
      if (!pokemon.types[1]) {
        typeEffectString = getTypeEmotes({
          type1: typeName,
          emotes: emotesAllowed,
        });
    ...
    if (pokemon.types[1]) {
      for (let [typeName, matchup] of Object.entries(type2)) {
        ...
        if (typeEffects[typeName] == 1 || typeEffects[typeName] == 2)
          superEffectives.push(typeEffectString);
        if (typeEffects[typeName] == -1 || typeEffects[typeName] == -2)
          resistances.push(typeEffectString);
        if (typeEffects[typeName] <= -3) immunities.push(typeEffectString);
      }
    }
    ...
    if (
      ...
      pokemonID = `${pokemonID}-${pokemon.types[0].toLowerCase()}`;
    // 3 digit IDs for now
    ...
    if (pokemon.heightm) {
      ...
      if (dynamaxBool)
        metricsString = metricsString.replace("m", "m+").replace("ft", "ft+");
    }
    let urlName = encodeURIComponent(
      pokemon.name.toLowerCase().replace(" ", "-"),
    );
    // Official art
    ...
    if (learnsetBool && learnsets[pokemon.id]) {
      ...
      // Prevo egg moves
      if (prevo) {
        for (let [moveName, learnData] of Object.entries(
          learnsets[prevo.id].learnset,
        )) {
          moveName = Dex.moves.get(moveName).name;
          for (let moveLearnData of learnData) {
    ...
    for (let levelMove in Object.entries(levelMoves))
      levelMovesString += `${levelMoves[levelMove][1]}: ${levelMoves[levelMove][0]}\n`;
    let tmMovesStrings = [];
    let tmMoveIndex = 0;
    for (const tmMove of tmMoves) {
      ...
      if (tmMovesStrings[tmMoveIndex].join(", ").length > 1000)
        tmMoveIndex += 1; // 1000 instead of 1024 to add an extra entry for the overflow
    }
    let eggMovesString = eggMoves.join(", ");
    let tutorMovesString = tutorMoves.join(", ");
    specialMoves = [...new Set(specialMoves)]
      .filter((el) => !levelMovesNames.includes(el))
      .filter((el) => !tmMoves.includes(el))
      .filter((el) => !eggMoves.includes(el))
      .filter((el) => !tutorMoves.includes(el));
    let specialMovesString = specialMoves.join(", ");
    let transferMovesStrings = [];
    let transferMoveIndex = 0;
    transferMoves = [...new Set(transferMoves)]
      .filter((el) => !levelMovesNames.includes(el))
      .filter((el) => !tmMoves.includes(el))
      .filter((el) => !eggMoves.includes(el))
      .filter((el) => !tutorMoves.includes(el))
      .filter((el) => !specialMoves.includes(el));
    for (const transferMove of transferMoves) {
      if (!transferMovesStrings[transferMoveIndex])
        transferMovesStrings[transferMoveIndex] = [];
      transferMovesStrings[transferMoveIndex].push(transferMove);
      if (transferMovesStrings[transferMoveIndex].join(", ").length > 1000)
        transferMoveIndex += 1;
    }
    let embedColor = globalVars.embedColor;
    if (pokemon.color) {
      if (colorHexes[pokemon.color.toLowerCase()])
        embedColor = colorHexes[pokemon.color.toLowerCase()];
    }
    ...
    previousPokemon = allPokemon.filter(
      (pokemon) => pokemon.num == previousPokemonID,
    )[0];
    nextPokemon = allPokemon.filter(
      (pokemon) => pokemon.num == nextPokemonID,
    )[0];
    ...
    if (previousPokemon)
      pkmButtons.addComponents(
        new Discord.MessageButton({
          customId: `pkmleft|${learnsetBool}|${shinyBool}`,
          style: "PRIMARY",
          emoji: "⬅️",
          label: previousPokemon.name,
        }),
      );
    ...
    if (pokemon.name !== pokemon.baseSpecies)
      pkmButtons.addComponents(
        new Discord.MessageButton({
          customId: `pkmbase|${learnsetBool}|${shinyBool}`,
          style: "PRIMARY",
          emoji: "⬇️",
          label: pokemon.baseSpecies,
        }),
      );
    if (nextPokemon)
      pkmButtons.addComponents(
        new Discord.MessageButton({
          customId: `pkmright|${learnsetBool}|${shinyBool}`,
          style: "PRIMARY",
          emoji: "➡️",
          label: nextPokemon.name,
        }),
      );
    if (pokemon.prevo) {
      ...
      if (
        pokemon.prevo !== previousPokemon.name &&
        pokemon.prevo !== nextPokemon.name
      )
        pkmButtons.addComponents(
          new Discord.MessageButton({
            customId: `pkmprevo|${learnsetBool}|${shinyBool}`,
            style: "PRIMARY",
            emoji: "⏬",
            label: pokemon.prevo,
          }),
    ...
    for (let i = 0; i < pokemon.evos.length; i++) {
      ...
      if (
        pokemon.evos[i] !== previousPokemon.name &&
        pokemon.evos[i] !== nextPokemon.name
      ) {
        if (pkmButtons.components.length < 5) {
          pkmButtons.addComponents(
            new Discord.MessageButton({
              customId: `pkmevo${i + 1}|${learnsetBool}|${shinyBool}`,
              style: "PRIMARY",
              emoji: "⏫",
              label: pokemon.evos[i],
            }),
          );
        } else {
          // This exists solely because of Eevee
          pkmButtons2.addComponents(
            new Discord.MessageButton({
              customId: `pkmevo${i + 1}|${learnsetBool}|${shinyBool}`,
              style: "PRIMARY",
              emoji: "⏫",
    ...
    if (pokemon.otherFormes && pokemon.otherFormes.length > 0)
      pokemonForms = pokemon.otherFormes;
      ...
      if (pokemonForms.length > 0) {
        if (pokemonForms.length < 6) {
          for (let i = 0; i < pokemonForms.length; i++) {
            formButtons.addComponents(
              new Discord.MessageButton({
                customId: `pkmForm${i}|${learnsetBool}|${shinyBool}`,
                style: "SECONDARY",
                label: pokemonForms[i],
              }),
            );
          }
        } else {
    ...
    const pkmEmbed = new Discord.MessageEmbed()
      .setColor(embedColor)
      .setAuthor({
        name: `${pokemonID.toUpperCase()}: ${pokemon.name}`,
        iconURL: iconAuthor,
      })
    ...
    if (superEffectives.length > 0)
      pkmEmbed.addField("Weaknesses:", superEffectives, false);
    if (resistances.length > 0)
      pkmEmbed.addField("Resistances:", resistances, false);
    if (immunities.length > 0)
      pkmEmbed.addField("Immunities:", immunities, false);
    pkmEmbed.addField(
      `Stats: ${statLevels}`,
      `HP: **${pokemon.baseStats.hp}** ${HPstats}
...
BST: ${pokemon.bst}`,
    ...
    // .setImage(banner)
    if (learnsetBool) {
      if (levelMovesString.length > 0)
        pkmEmbed.addField("Levelup Moves:", levelMovesString, false);
      tmMovesStrings.forEach((tmMovesString) =>
        pkmEmbed.addField("TM Moves:", tmMovesString.join(", "), false),
      );
      if (eggMovesString.length > 0)
        pkmEmbed.addField("Egg Moves:", eggMovesString, false);
      if (tutorMovesString.length > 0)
        pkmEmbed.addField("Tutor Moves:", tutorMovesString, false);
      if (specialMovesString.length > 0)
        pkmEmbed.addField("Special Moves:", specialMovesString, false);
    ...
    function calcHP(base) {
      ...
      let max100 = Math.floor(
        ((2 * base + 31 + 252 / 4) * 100) / 100 + 100 + 10,
      );
      //// Let's Go
      // let min50 = Math.floor((((2 * base) * 50) / 100) + 50 + 10);
      ...
      if (pokemon.name.endsWith("-Gmax") || pokemon.name.endsWith("-Eternamax"))
        StatText = `(${Math.floor(min50 * 1.5)}-${max50 * 2}) (${Math.floor(
          min100 * 1.5,
        )}-${max100 * 2})`;
    ...
    function calcStat(base) {
      ...
      let max50 = Math.floor(
        (((2 * base + 31 + 252 / 4) * 50) / 100 + 5) * 1.1,
      );
      let min100 = Math.floor(((2 * base * 100) / 100 + 5) * 0.9);
      let max100 = Math.floor(
        (((2 * base + 31 + 252 / 4) * 100) / 100 + 5) * 1.1,
      );
      //// Let's Go
