module.exports = async (client, interaction, monsterData, ephemeral) => {
  ...
  try {
    ...
    let iconsRepo =
      "https://github.com/CrimsonNynja/monster-hunter-DB/blob/master/icons/";
    ...
    monsterData.games.forEach((game) => {
      ...
      if (
        game.game == mostRecentMainlineGame ||
        game.game == fallbackGame1 ||
        game.game == fallbackGame2
      ) {
    ...
    if (!monsterIcon)
      monsterIcon = `${iconsRepo}${mostRecentGameEntry.image}?raw=true`;
    ...
    let isInImageDBGame =
      gameAppearances.includes(MHRise) ||
      gameAppearances.includes(MHW) ||
      gameAppearances.includes(MHGU);
    let isOnlyInGU =
      !gameAppearances.includes(MHRise) &&
      !gameAppearances.includes(MHW) &&
      gameAppearances.includes(MHGU);
    let newestGameIsWorld =
      !gameAppearances.includes(MHRise) && gameAppearances.includes(MHW);
    if (isInImageDBGame) {
      gameDBName = "MHRise";
      ...
      if (monsterURLName == "Narwa_the_Allmother")
        monsterURLName = "Narwa_The_Allmother"; // wack as fuck
      if (isOnlyInGU) {
        gameDBName = "MHGU";
      ...
      if (gameAppearances.includes(MHRise))
        monsterURLName = `${monsterURLName}_HZV`;
      monsterBanner = `https://github.com/RoboMechE/${gameDBName}-Database/blob/${gameDBBranchName}/${monsterSize}/${encodeURIComponent(
        monsterURLName,
      )}.png?raw=true`;
    }
    let monsterGameIndicator = gameDBName;
    if (monsterIcon)
      monsterGameIndicator = monsterIcon.replace(iconsRepo, "").split("-")[0];
    let monsterRenderName = `${monsterGameIndicator}-${monsterData.name.replaceAll(
      " ",
      "_",
    )}_Render_001.png`;
    ...
    if (!renderExists) {
      if (monsterGameIndicator == "MHGU" || monsterGameIndicator == "MH4U") {
        ...
        monsterRenderName = monsterRenderName.replace(
          "Render_001",
          "Render_002",
        );
        monsterRender = getWikiURL(monsterRenderName, mhWiki);
    ...
    if (monsterData.elements) {
      monsterData.elements.forEach((element) => {
        ...
        if (emotesAllowed)
          elementString = `${elementEmotes[element]}${element}`;
    ...
    if (monsterData.subSpecies && monsterData.subSpecies.length > 0) {
      if (monsterData.subSpecies.length < 6) {
        for (let i = 0; i < monsterData.subSpecies.length; i++) {
          subSpeciesButtons.addComponents(
            new Discord.MessageButton({
              customId: `mhSub${i}`,
              style: "SECONDARY",
              label: monsterData.subSpecies[i],
            }),
          );
        }
      } else {
    ...
    if (subSpeciesButtons.components.length > 0)
      buttonArray.push(subSpeciesButtons);

    let mhEmbed = new Discord.MessageEmbed()
      .setColor(globalVars.embedColor)
      .setAuthor({
        name: `${monsterData.name} (${monsterData.type})`,
        iconURL: monsterIcon,
      })
    ...
    if (monsterElements.length > 0)
      mhEmbed.addField("Element:", monsterElements, true);
    if (monsterWeaknesses.length > 0)
      mhEmbed.addField("Weakness:", monsterWeaknesses, true);
    if (monsterAilments.length > 0)
      mhEmbed.addField("Ailment:", monsterAilments, true);
    mhEmbed
