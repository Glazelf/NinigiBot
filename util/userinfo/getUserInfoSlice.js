...
module.exports = async (client, interaction, page, user) => {
  ...
  if (member)
    serverAvatar = member.displayAvatarURL(globalVars.displayAvatarSettings);
  ...
  let profileButtons = new Discord.MessageActionRow().addComponents(
  ...
  if (page > 0)
    profileButtons.addComponents(
      new Discord.MessageButton({
        customId: `usf${page - 1}:${user.id}`,
        style: "PRIMARY",
        emoji: "⬅️",
      }),
    );
  ...
  let user_db = await api_user.getUser(user.id, [
    "swcode",
    "money",
    "birthday",
    "user_id",
  ...
  switch (page) {
    case 0:
      ...
      if (member)
        memberRoles = member.roles.cache.filter(
          (element) => element.name !== "@everyone",
        );
      let rolesSorted = "None";
      let shortenedRoles;
      if (memberRoles && memberRoles.size !== 0) {
        rolesSorted = await memberRoles.sort(
          (r, r2) => r2.position - r.position,
        );
      ...
      let banner = null;
      if (user.banner)
        banner = user.bannerURL(globalVars.displayAvatarSettings);
      // Profile badges
      let badgesArray = [];
      let badgesString = "";
      if (
      ...
      let joinPercentage = Math.ceil(
        (joinRank / interaction.guild.memberCount) * 100,
      );
      ...
      if (birthday && birthdayParsed && member)
        profileEmbed.addField("Birthday:", birthdayParsed, true);
      if (switchCode && switchCode !== "None" && member)
        profileEmbed.addField("Switch FC:", switchCode, true);
      if (joinRank) profileEmbed.addField("Join Ranking:", joinRankText, true);
      if (memberRoles)
        profileEmbed.addField(`Roles: (${roleCount})`, rolesSorted, false);
      profileEmbed.addField(
        "Created:",
        `<t:${Math.floor(user.createdAt.valueOf() / 1000)}:f>`,
        true,
      );
      if (member)
        profileEmbed.addField(
          "Joined:",
          `<t:${Math.floor(member.joinedAt.valueOf() / 1000)}:R>`,
          true,
        );
      if (member && member.premiumSince > 0)
        profileEmbed.addField(
          `Boosting Since:`,
          `<t:${Math.floor(member.premiumSince.valueOf() / 1000)}:R>`,
          true,
        );
