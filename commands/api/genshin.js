exports.run = async (client, interaction) => {
  ...
  try {
    ...
    let response;
    let buttonArray = [];
    let giEmbed = new Discord.MessageEmbed().setColor(globalVars.embedColor);
    switch (interaction.options.getSubcommand()) {
      case "character":
        giAPI += `characters/`;
        let characterName = interaction.options
          .getString("character")
          .toLowerCase();
        let detailed = false;
        ...
        if (response.status != 200)
          return sendMessage({
            ...
            content: `Error occurred, make sure that character exists.`,
          });
        ...
        if (character.birthday) {
          ...
          characterBirthday = parseDate(
            `${characterBirthdayArray[2]}${characterBirthdayArray[1]}`,
          );
        }
        giEmbed
        ...
        if (character.birthday)
          giEmbed.addField("Birthday:", characterBirthday, true);
        if (detailed) {
          ...
          await character.skillTalents.forEach((skill) => {
            ...
            if (skillDesc.length <= 1024) {
              ...
              giEmbed.addField(
                `${skill.name} (Active)`,
                `${skillDesc.substring(0, 1024)}...`,
                false,
              );
              giEmbed.addField(
                `${skill.name} (Active) (cont.)`,
                `...${skillDesc.substring(1024)}`,
                false,
              );
            }
          });
          ...
          await character.constellations.forEach((constellation) => {
            giEmbed.addField(
              `${constellation.name} (${constellation.unlock})`,
              constellation.description.replace("\n\n", "\n"),
              false,
            );
          });
        }
      ...
      case "weapon":
        ...
        if (weapon.subStat !== "-")
          giEmbed.addField("Substat:", weapon.subStat, true);
        if (weapon.passiveName !== "-")
          giEmbed.addField(
            `${weapon.passiveName} (Passive)`,
            weapon.passiveDesc,
            false,
          );
        break;
      case "artifact":
        giAPI += `artifacts/`;
        let artifactName = interaction.options
          .getString("artifact")
          .toLowerCase();
        ...
        if (artifact["1-piece_bonus"])
          giEmbed.addField("1-Piece Bonus:", artifact["1-piece_bonus"], false);
        if (artifact["2-piece_bonus"])
          giEmbed.addField("2-Piece Bonus:", artifact["2-piece_bonus"], false);
        if (artifact["3-piece_bonus"])
          giEmbed.addField("3-Piece Bonus:", artifact["3-piece_bonus"], false);
        if (artifact["4-piece_bonus"])
          giEmbed.addField("4-Piece Bonus:", artifact["4-piece_bonus"], false);
        if (artifact["5-piece_bonus"])
          giEmbed.addField("5-Piece Bonus:", artifact["5-piece_bonus"], false);
        break;
    }
    return sendMessage({
      client: client,
