exports.run = async (client, interaction) => {
  ...
  try {
    ...
    const learnsets =
      require("../../node_modules/pokemon-showdown/dist/data/learnsets.js").Learnsets;
    ...
    if (
      ephemeral == true &&
      !interaction.guild.members.me.permissions.has("USE_EXTERNAL_EMOJIS") &&
      !adminBot
    )
      emotesAllowed = false;
    // Bools
    ...
    // Variables
    let pokemonEmbed = new Discord.MessageEmbed().setColor(
    ...
    switch (interaction.options.getSubcommand()) {
      ...
      case "ability":
        ...
        if (
          ...
          ability.isNonstandard == "CAP"
        )
          return sendMessage({
            client: client,
            interaction: interaction,
            content: `Sorry, I could not find an ability by that name.`,
          });
        ...
        let abilityMatches = Object.values(allPokemon).filter(
          (pokemon) =>
            Object.values(pokemon.abilities).includes(ability.name) &&
            pokemon.exists &&
            pokemon.num > 0,
        );
        let abilityMatchesString = "";
        abilityMatches.forEach(
          (match) => (abilityMatchesString += `${match.name}, `),
        );
        ...
        if (abilityMatchesString.length > 0)
          pokemonEmbed.addField("Pokémon:", abilityMatchesString, false);
      ...
      case "item":
        ...
        if (!item || !item.exists)
          return sendMessage({
            client: client,
            interaction: interaction,
            content: `Sorry, I could not find an item by that name.`,
          });
      ...
      // Moves
      case "move":
        if (!move || !move.exists || move.isNonstandard == "CAP")
          return sendMessage({
            client: client,
            interaction: interaction,
            content: `Sorry, I could not find a move by that name.`,
          });
        ...
        for await (const [key, value] of Object.entries(learnsets)) {
          ...
          if (
            value.learnset[move.id].some((learnstring) =>
              learnstring.startsWith(currentGeneration),
            )
          )
            moveLearnPool.push(pokemonMatch.name);
        }
        let moveLearnPoolString = moveLearnPool.join(", ");
        if (moveLearnPoolString.length > 1024)
          moveLearnPoolString = `${moveLearnPool.length} Pokémon!`;
        ...
        if (move.flags.contact)
          description += " Makes contact with the target.";
        ...
        pokemonEmbed.setAuthor({ name: moveTitle }).setDescription(description);
        if (move.basePower > 1 && !move.isMax)
          pokemonEmbed.addField("Power:", move.basePower.toString(), true);
        if (target !== "Self")
          pokemonEmbed.addField("Accuracy:", accuracy, true);
        pokemonEmbed
          .addField("Type:", type, true)
          .addField("Category:", category, true)
          .addField("Target:", target, true);
        if (move.critRatio !== 1)
          pokemonEmbed.addField("Crit Rate:", move.critRatio.toString(), true);
        if (!move.isMax) pokemonEmbed.addField("PP:", ppString, true);
        if (move.priority !== 0)
          pokemonEmbed.addField("Priority:", move.priority.toString(), true);
        ...
        if (moveLearnPool.length > 0)
          pokemonEmbed.addField(
            `Learned By (Gen ${currentGeneration}):`,
            moveLearnPoolString,
            false,
          );
        break;
      ...
      case "nature":
        ...
        if (!nature || !nature.exists)
          return sendMessage({
            client: client,
            interaction: interaction,
            content: `Sorry, I could not find a nature by that name.`,
          });
      ...
      case "format":
        ...
        if (!format || !format.exists)
          return sendMessage({
            client: client,
            interaction: interaction,
            content: `Sorry, I could not find a format by that name.`,
          });

        if (format.threads) {
          format.threads.forEach((thread) => {
            pokemonButtons.addComponents(
              new Discord.MessageButton({
                label: thread.split(">")[1].split("<")[0],
                style: "LINK",
                url: thread.split('"')[1],
              }),
            );
        ...
        let ruleset = null;
        if (format.ruleset && format.ruleset.length > 0)
          ruleset = format.ruleset.join(", ");
        let banlist = null;
        if (format.banlist && format.banlist.length > 0)
          banlist = format.banlist.join(", ");
        let unbanlist = null;
        if (format.unbanlist && format.unbanlist.length > 0)
          unbanlist = format.unbanlist.join(", ");
        ...
        if (format.restricted && format.restricted.length > 0)
          pokemonEmbed.addField(
            "Restricted type:",
            format.restricted.join(", "),
            false,
          );
        break;
      // Pokémon
      case "pokemon":
        if (!pokemon || !pokemon.exists || pokemon.num <= 0)
          return sendMessage({
            client: client,
            interaction: interaction,
            content: noPokemonString,
          });
        let messageObject = await getPokemon({
          client: client,
          interaction: interaction,
          pokemon: pokemon,
          learnsetBool: learnsetBool,
          shinyBool: shinyBool,
          ephemeral: ephemeral,
        });
        return sendMessage({
      ...
      case "learn":
        ...
        if (learnsets[pokemon.id]) {
          ...
          if (learnInfo.length == 0)
            learnAuthor = `${pokemon.name} does not learn ${move.name}`;
        } else
          return sendMessage({
            client: client,
            interaction: interaction,
            content: `I could not find a learnset for ${pokemon.name}.`,
          });
        pokemonEmbed.setAuthor({ name: learnAuthor }).setDescription(learnInfo);
        return sendMessage({
          client: client,
          interaction: interaction,
          embeds: pokemonEmbed,
      ...
      case "usage":
        ...
        if (formatInput == "gen7vgc2019")
          formatInput = "gen7vgc2019ultraseries";

        let rating = 0;
        let ratingTresholds = [0, 1500, 1630, 1760];
        if (formatInput.match(/gen.{1,2}(ou)$/g))
          ratingTresholds = [0, 1500, 1695, 1825]; // OU has different rating tresholds
        ...
        try {
          ...
          genericUsageResponse = await axios.get(
            `https://www.smogon.com/stats/${year}-${stringMonth}/${formatInput}-${rating}.txt`,
          );
        } catch (e) {
          ...
          return sendMessage({
            ...
            interaction: interaction,
            content: replyText,
            components: usageButtons,
          });
        }
        // Filter, split and trim pokemon data
        let usageArray = response.data
          .replaceAll("|", "")
          .replaceAll("\n", "")
          .trim()
          .split(
        ...
        await Object.keys(usageArray).forEach((key) => {
          usageArray[key] = usageArray[key]
            .replaceAll("+", "")
            .replaceAll("--", "");
        });
        usageArray = usageArray.map((element) => element.trim());
        // Variables for generic usage data
        let totalBattleCount = genericUsageResponse.data
          .split("battles: ")[1]
          .split("Avg.")[0]
          .replace("\n", "")
          .trim();
        ...
        let usageEmbed = new Discord.MessageEmbed().setColor(
          globalVars.embedColor,
        );
        if (pokemonName) {
          let usagePokemonString = usageArray.find((element) =>
            element.startsWith(pokemonName),
          );
          if (!usagePokemonString)
            return sendMessage({
              client: client,
              interaction: interaction,
              content: `Could not find any data for ${pokemonName} in ${formatInput} during the specified month.`,
              components: usageButtons,
            });
          // Data from generic usage page
          genericDataSplitPokemon =
            genericUsageResponse.data.split(pokemonName);
          pokemonDataSplitLine = genericDataSplitPokemon[1].split("|");
          rawUsage = pokemonDataSplitLine[2].trim();
          usagePercentage = `${
            Math.round(pokemonDataSplitLine[1].trim().replace("%", "") * 100) /
            100
          }%`;
          ...
          let abilitiesString = usagePokemonString
            .split("Abilities")[1]
            .split("Items")[0]
            .split("%")
            .map(function (x) {
            ...
            .join("%\n")
            .replaceAll("   ", "");
          let itemsString = usagePokemonString
            .split("Items")[1]
            .split("Spreads")[0]
            .split("%")
            .map(function (x) {
            ...
            .join("%\n")
            .replaceAll("   ", "");
          let spreadsString = usagePokemonString
            .split("Spreads")[1]
            .split("Moves")[0]
            .split("%")
            .map(function (x) {
            ...
            .replaceAll("   ", "")
            .replaceAll(":", " ");
          let movesString = usagePokemonString
            .split("Moves")[1]
            .split("Teammates")[0]
            .split("%")
            .map(function (x) {
            ...
            .join("%\n")
            .replaceAll("   ", "");
          let teammatesString = usagePokemonString
            .split("Teammates")[1]
            .split("Checks and Counters")[0]
            .split("%")
            .map(function (x) {
              return x.trim();
            })
            .join("%\n")
            .replaceAll("   ", "");
          let countersString = usagePokemonString
            .split("Checks and Counters")[1]
            .split("out)")
            .map(function (x) {
              return x.trim();
            })
          ...
          usageEmbed
            .setAuthor({
              name: `${pokemonName} ${formatInput} ${rating}+ (${stringMonth}/${year})`,
            })
            .setDescription(
              `#${usageRank} | ${usagePercentage} | ${rawUsage} uses`,
            )
          ...
          if (countersString.length > 0)
            usageEmbed.addField("Checks and Counters:", countersString, true);
        } else {
          ...
          await usageArray.forEach((element) => {
            ...
            usagePercentage = `${
              Math.round(
                pokemonDataSplitLine[1].trim().replace("%", "") * 100,
              ) / 100
            }%`;
            usageList.push(
              `${usageListIndex}.${pokemonName} ${usagePercentage}`,
            );
            usageListIndex++;
          });
          let usageListPart1 = [];
          let usageListPart2 = [];
          await usageList.forEach((element) => {
            if (usageListPart1.length < 50) usageListPart1.push(element);
            else if (usageListPart2.length < 50) usageListPart2.push(element);
          });
          usageEmbed
            .setAuthor({
              name: `Usage for ${formatInput} ${rating}+ (${stringMonth}/${year})`,
            })
    ...
    function getLearnData(learnData) {
      ...
      learnData.forEach((learnMethod) => {
        ...
        switch (learnType) {
          case "L":
            learnInfo += `Gen ${learnGen}: Level ${
              learnMethod.split("L")[1]
            }\n`;
            break;
          case "M":
