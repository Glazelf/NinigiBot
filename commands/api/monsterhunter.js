exports.run = async (client, interaction) => {
  ...
  try {
    ...
    let buttonArray = [];
    let mhEmbed = new Discord.MessageEmbed().setColor(globalVars.embedColor);
    ...
    switch (interaction.options.getSubcommand()) {
      ...
      case "quest":
        ...
        if (!questData)
          return sendMessage({
            ...
            content: "Could not find the specified quest.",
          });
        // Format quest title
      ...
      case "quests":
        ...
        let questsTotal = questsJSON.quests.filter(
          (quest) => quest.game == gameName,
        );
        if (questsTotal.length == 0)
          return sendMessage({
            ...
            interaction: interaction,
            content:
              "Could not find any quests for that game. If you are certain this game exists the quest list may still be a work in progress.",
          });
        // Sort by difficulty
      ...
      // Monsters
      case "monster":
        let monsterName = interaction.options
          .getString("monster")
          .toLowerCase();
        // Get monster
        let monsterData;
        if (monsterName == "random") {
          ...
          monstersJSON.monsters.forEach((monster) => {
            if (monster.name.toLowerCase() == monsterName)
              monsterData = monster;
          });
        }
        if (!monsterData)
          return sendMessage({
            client: client,
            interaction: interaction,
            content: "Could not find the specified monster.",
          });

        let messageObject = await getMonster(
          client,
          interaction,
          monsterData,
          ephemeral,
