exports.run = async (client, message, args = []) => {
    ...
    try {
        ...
        // Split off command
        if (!args[0]) return sendMessage({ client: client, message: message, content: `Please provide text to say.` });
        ...
        if (textMessage.length < 1) {
            return sendMessage({ client: client, message: message, content: `You need to specify text for me to say.` });
        };
        ...
        if (message.member.id == client.config.ownerID) {
            try {
                ...
                return sendMessage({ client: client, message: message, content: `Message succesfully sent to specified channel.` });
            } catch (e) {
            ...
            return sendMessage({ client: client, message: message, content: textMessage });
        };
