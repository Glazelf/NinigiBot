exports.run = async (client, interaction) => {
  ...
  try {
    ...
    if (timezone != 0)
      targetDate.setTime(targetDate.getTime() + timezone * 60 * 60 * 1000); // Add timezone difference
    let dateString = `${currentDate.getUTCDate()} ${targetDate.toLocaleString(
      "default",
      { month: "long" },
    )} ${targetDate.getUTCFullYear()} at ${targetDate.getUTCHours()}:${targetDate.getUTCMinutes()} UTC`;
