  ...
  try {
    ...
    switch (interaction.options.getSubcommand()) {
      case "addshoptrophy":
        ...
        let trophy_emote = interaction.options
          .getString("emote")
          .trim()
          .replace(/^:+/, "")
          .replace(/:+$/, "");
        ...
        if (!parsed_emote) {
          ...
          if (!parsed_emote)
            error +=
              "Emote is not a valid Unicode Emoji or Discord custom emote";
        }
        ...
        if (error.length > 0) {
          returnString =
            "Could not add the trophy due to the following issues:```\n" +
            error +
            "\n```";
        } else {
          await api_trophy.createShopTrophy(
            trophy_name,
            trophy_emote,
            trophy_desc,
            trophy_price,
      ...
      case "deleteshoptrophy":
        ...
        returnString = res
          ? `${trophy_name} deleted successfully from the shop!`
          : `${trophy_name} does not exist in the __shop__`;
        return sendMessage({
          client: client,
          interaction: interaction,
          content: returnString,
...
module.exports.config = {
  name: "manager",
  description:
    "Owner only, manage multiple aspects about Ninigi Virtual Simulation Core",
      ...
      options: [
        {
          ...
          type: "STRING",
          description:
            "Name of the trophy. Make sure it's unique with less than 25 characters!",
          required: true,
        },
        {
          name: "emote",
