...
export default async (interaction, ephemeral) => {
    try {
        ...
        try {
            ...
            // console.log(e);
            return sendMessage({ client: interaction.client, interaction: interaction, content: `Error occurred:\n${codeBlock(e.stack)}` });
        };
        ...
        for (const [key, value] of Object.entries(config)) {
            if (evaled.includes(value) && ephemeral == false) return sendMessage({ client: interaction.client, interaction: interaction, content: `For security reasons this content can't be returned.` });
        };
