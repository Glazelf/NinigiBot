  ...
  try {
    ...
    if (userIDArg || channelIDArg) {
      ...
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `Please provide a user ID or channel ID.`,
      });
    }
    ...
    if (!target)
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `I could not find a user or channel with that ID.`,
      });
    let targetFormat = null;
    if (channelIDArg)
      targetFormat = `${target.name} (${target.id}) in **${target.guild.name}** (${target.guild.id})`;
    ...
    try {
      ...
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `Message sent to ${targetFormat}.`,
      });
    } catch (e) {
      // console.log(e);
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `Failed to message ${targetFormat}`,
      });
    }
  } catch (e) {
    // Log error
