exports.run = async (client, interaction) => {
  ...
  try {
    ...
    await sendMessage({
      client: client,
      interaction: interaction,
      content: "Deleting outdated entries...",
    });
    ...
    users.forEach((user) => {
      if (
        !server_users.includes(user.user_id) ||
        (!user.swcode && !user.birthday && user.money < 100)
      ) {
    ...
    if (deleted_users.length == 0) {
      return sendMessage({
        client: client,
        interaction: interaction,
        content: "Database is already clean!",
      });
    }
  ...
  options: [
    {
      ...
      type: "BOOLEAN",
      description:
        "Are you sure? This is an irreversible and expensive command.",
    },
  ],
};
