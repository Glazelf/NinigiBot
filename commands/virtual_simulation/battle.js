  ...
  try {
    ...
    if (target.bot)
      return sendMessage({
        ...
        content: `You can not battle a bot.`,
      });
    ...
    if (!trainers[1])
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `Please tag a valid person to battle.`,
      });
    if (trainers[0].id === trainers[1].id)
      return sendMessage({
        client: client,
        interaction: interaction,
    ...
    if (globalVars.battling.yes)
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `Theres already a battle going on.`,
      });
    let shinxes = [];
    ...
    const avatars = [
      trainers[0].displayAvatarURL(globalVars.displayAvatarSettings),
      trainers[1].displayAvatarURL(globalVars.displayAvatarSettings),
    ];
    ...
    ctx.beginPath();
    for (let i = 0; i < 2; i++)
      ctx.arc(47 + 147 * i, 36, 29, 0, Math.PI * 2, false);
    ctx.closePath();
    ctx.clip();
    ...
    const filter = (interaction) =>
      (interaction.customId === "yes_battle" ||
        interaction.customId === "no_battle") &&
      interaction.user.id === trainers[1].id;
    let trainer_answer;
    try {
      trainer_answer = await sent_message.awaitMessageComponent({
        filter,
        time: 25_000,
      });
    } catch {
    ...
    if (!trainer_answer) {
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `Battle cancelled, the challenge timed out.`,
        components: [],
      });
      ...
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `Battle cancelled, ${trainers[1]} rejected the challenge.`,
        components: [],
      });
    }
    if (globalVars.battling.yes)
    ...
    const image_nicks = [];
    for (let i = 0; i < 2; i++)
      shinxes[i].nick.trim().toLowerCase() === "shinx"
        ? image_nicks.push(`${shinxes[i].owner.username}'s Shinx`)
        : image_nicks.push(shinxes[i].nick);
    for (let i = 0; i < 2; i++) {
    ...
    for (let i = 0; i < 2; i++)
      nicks.push(`${shinxes[i].owner.username}'s ${shinxes[i].nick}`);
    ...
    while (true) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        if (result === true) {
          ...
          ctx.beginPath();
          for (let i = 0; i < 2; i++)
            ctx.arc(58 + 134 * i, 83, 40, 0, Math.PI * 2, false);
          ctx.closePath();
          ctx.clip();
          for (let q = 0; q < 2; q++) {
          ...
          for (let h = 0; h < 2; h++) {
            ...
            const exp = shinxes[h].gainExperience(
              shinxes[(h + 1) % 2].level,
              i !== h,
            );
            ...
            if (exp[1] > 0) {
              text += addLine(
                `**${nicks[h]}** grew to level **${shinxes[h].level}**!`,
              );
            }
          }

          for (let p = 0; p < 2; p++)
            await shinxApi.saveBattle(shinxes[p], p === i);
          ...
          if (result === -1) {
            text += addLine(
              `**${nicks[i]}** lost his shield by blocking a deathblow!`,
            );
          }
        }
      }
