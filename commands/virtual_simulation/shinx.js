...
exports.run = async (client, interaction) => {
  ...
  try {
    ...
    if (
      ephemeral == true &&
      !interaction.guild.roles.everyone.permissions.has("USE_EXTERNAL_EMOJIS")
    )
      emotesAllowed = false;
    ...
    switch (interaction.options.getSubcommand()) {
      case "info":
        ...
        if (belly_prop > 0) {
          ctx.fillStyle = require("../../util/shinx/getBelly")(
            shinx.getBellyProportion(),
          );
      ...
      case "feed":
        ...
        switch (res) {
          ...
          case "Ok":
            ...
            guests = await shinxApi.getRandomShinx(
              2,
              shinx.user_id,
              interaction.guild,
            );
            ...
            for (let i = 0; i < guests.length; i++) {
              const nick = userFinder
                .get(guests[i].user_id)
                .user.username.split(" ");
              ctx.drawImage(
                img,
                51 * !guests[i].user_male,
                72 * 2,
                51,
                72,
                298,
        ...
        for (let i = 0; i < guests.length; i++) {
          ...
          for (let k = nick.length - 1; 0 <= k; k--) {
            ...
            ctx.fillText(
              nick[k],
              layout[i][0][1],
              layout[i][0][2] - 19 * (nick.length - 1 - k),
            );
          }
        }
      ...
      case "shiny":
        ...
        if (res) {
          ...
          returnString = is_shiny
            ? `Your shinx is shiny now`
            : `Your shinx is no longer shiny`;
          ...
          returnString =
            "You need that your shinx arrives to level 50 for that.";
          messageFile = null;
        }
        return sendMessage({
          client: client,
      ...
      case "release":
        ...
        if (!confirm)
          return sendMessage({
            client: client,
            interaction: interaction,
            content: `This action is irreversible and will reset all your Shinx's values.\nPlease set the \`confirm\` option for this command to \`true\` if you're sure.`,
          });
        shinx = await shinxApi.getShinx(master.id);
        await shinx.destroy();
        ...
        return sendMessage({
          client: client,
          interaction: interaction,
          content: `Released Shinx and reset all it's values.`,
        });
    }
  } catch (e) {
    // Log error
