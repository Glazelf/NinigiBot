    ...
    if (!requestRole) {
      ...
      if (roleText.length <= selectOptionLimit) {
        await db.forEach(async (eligibleRole) => {
          let currentRole = await interaction.guild.roles.fetch(
            eligibleRole.role_id,
          );
        ...
        roles = Object.entries(roles).sort(
          (a, b) => b[1].role.position - a[1].role.position,
        );
        ...
        if (rolesArray.length < 1)
          return sendMessage({
            ...
            content: noRolesString,
          });

        let rolesSelects = new Discord.MessageActionRow().addComponents(
          new Discord.MessageSelectMenu({
            customId: "role-select",
            placeholder: "Click here to drop down!",
            options: rolesArray,
            maxValues: rolesArray.length,
          }),
        );

        let returnString = `Choose roles to toggle:`;
        if (ephemeral == true)
          returnString = `${rolesArray.length}/25 roles before the dropdown is full.\n${removeEmote} You have the role and it will be removed.\n${receiveEmote} You don't have this role yet and it will be added.\n${returnString}`;
        return sendMessage({
          client: client,
          interaction: interaction,
          content: returnString,
          components: rolesSelects,
          ephemeral: ephemeral,
        });
      }
      // Help menu
      ...
      if (roleHelpMessage.length > embedDescriptionCharacterLimit)
        return sendMessage({
          client: client,
          interaction: interaction,
          content: `Embed descriptions can't be over ${embedDescriptionCharacterLimit} characters. Consider removing some roles.`,
        });
      ...
      if (!requestRole || !roleIDs.includes(requestRole.id))
        return sendMessage({
          client: client,
          interaction: interaction,
          content: invalidRoleText,
        });
      if (requestRole.managed == true)
        return sendMessage({
          client: client,
          interaction: interaction,
          content: `I can't manage ${requestRole.name} because it is being automatically managed by an integration.`,
        });
      if (
        ...
        return sendMessage({
          client: client,
          interaction: interaction,
          content: `I can't manage ${requestRole} because it is above my highest role.`,
        });
