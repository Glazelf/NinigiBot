exports.run = async (client, interaction) => {
  ...
  try {
    ...
    if (!switchFC) {
      if (switchCodeGet)
        return sendMessage({
          ...
          content: `${interaction.user.username}'s Nintendo Switch friend code is ${switchCodeGet}.`,
          ephemeral: false,
        });
      return sendMessage({
        client: client,
    ...
    switchFC = /^(?:SW)?[- ]?([0-9]{4})[- ]?([0-9]{4})[- ]?([0-9]{4})$/.exec(
      switchFC,
    );
    if (!switchFC)
      return sendMessage({
    ...
    return sendMessage({
      client: client,
      interaction: interaction,
      content: `Updated your Nintendo Switch friend code.`,
    });
  } catch (e) {
    // Log error
