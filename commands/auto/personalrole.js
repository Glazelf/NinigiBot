exports.run = async (client, interaction) => {
  ...
  try {
    ...
    const {
      PersonalRoles,
      PersonalRoleServers,
    } = require("../../database/dbServices/server.api");
    ...
    let serverID = await PersonalRoleServers.findOne({
    ...
    if (!serverID)
      return sendMessage({
        ...
        content: `Personal Roles are disabled in **${interaction.guild.name}**.`,
      });

    let roleDB = await PersonalRoles.findOne({
      where: { server_id: interaction.guild.id, user_id: interaction.user.id },
    });
    ...
    if (
      interaction.guild.premiumSubscriptionCount >= nitroLevel2Req ||
    ...
    if (!boosterRole)
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `**${interaction.guild}** does not have a Nitro Booster role. This role is created the first time someone boosts the server.`,
      });
    ...
    if (
      ...
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `You need to be a Nitro Booster or moderator to manage a personal role.`,
      });
    ...
    if (
      ...
      personalRolePosition
    )
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `My highest role isn't above your personal role or the Nitro Boost role so I can't edit your personal role.`,
      });
    if (roleColor) {
      ...
      if (roleColor.length > 6)
        roleColor = roleColor.substring(roleColor.length - 6, roleColor.length);
      while (roleColor.length < 6) roleColor = "0" + roleColor;
    }
    if (deleteBool == true)
      return deleteRole(
        `Deleted your personal role and database entry.`,
        `Your personal role isn't in my database so I can't delete it.`,
      );
    ...
    if (!boosterRole && !modBool && !adminBool)
      return deleteRole(
        `Since you can't manage a personal role anymore I cleaned up your old role.`,
        `You need to be a Nitro Booster or moderator to manage a personal role.`,
      );

    if (roleDB) {
      let editReturnString = `Updated your role.`;
      let personalRole = interaction.guild.roles.cache.find(
        (r) => r.id == roleDB.role_id,
      );
      if (!personalRole) return createRole();
      if (!colorArg) roleColor = personalRole.color;
      if (roleColor != personalRole.color)
        ...
        if (iconSize > roleIconSizeLimit) {
          ...
          try {
            await personalRole.setIcon(iconImg, [
              `Personal role image update requested by ${interaction.user.username} (${interaction.user.id}).`,
            ]);
            editReturnString += `\n- Image updated.`;
          } catch (e) {
            // console.log(e);
      ...
      if (
        ...
        interaction.member.roles.add(personalRole.id);

      return sendMessage({
        client: client,
        interaction: interaction,
        content: editReturnString,
      });
    } else {
    ...
    async function createRole() {
      ...
      let oldEntry = await PersonalRoles.findOne({
        where: {
      ...
      try {
        ...
        if (e.toString().includes("Missing Permissions")) {
          ...
          return sendMessage({
            client: client,
            interaction: interaction,
            content: `An error occurred creating a role.`,
          });
        }
      }
      let createdRole = await interaction.guild.roles.cache.find(
      ...
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `Created a personal role for you.`,
      });
    }

    async function deleteRole(successString, failString) {
      if (roleDB) {
        let oldRole = interaction.guild.roles.cache.find(
          (r) => r.id == roleDB.role_id,
        );
