...
exports.run = async (client, message, args = []) => {
    ...
    try {
        ...
        if (message.mentions && (message.mentions.members.size > 0 || message.mentions.repliedUser)) {
            ...
            if (target.bot) return sendMessage({ client: client, message: message, content: `**${target.tag}** is a bot.` });
            ...
            if (expectedId && expectedId[1] == targetId) {
                ...
                args.splice(0, 1);
            } else return sendMessage({ client: client, message: message, content: `The syntax is \`${prefix}shinx <target> <usual command>\`.` });
        } else {
        ...
        if (args[0] === 'level') {
            ...
            else return sendMessage({ client: client, message: message, content: `Please specify a valid number.` });
            return sendMessage({ client: client, message: message, content: `Shinx leveled up to level ${shinx.levelUp(parseInt(level))}` });

        } else if (args[0] === 'gender') {
            return shinx.trans() ? sendMessage({ client: client, message: message, content: `Your character is now male, ${master}!` }) : sendMessage({ client: client, message: message, content: `Your character is now female, ${master}!` });
            ...
            return sendMessage({ client: client, message: message, files: messageFile });
            ...
            return sendMessage({ client: client, message: message, content: `**${shinx.nick}** ${reaction[0]}`, files: welcomeFile });
            ...
            if (nickname.length < 2 || nickname.length > 10) return sendMessage({ client: client, message: message, content: `Please specify a valid nickname between 2 and 10 characters.` });
            ...
            if (shinyCharm.length < 1) return sendMessage({ client: client, message: message, content: `You need a Shiny Charm to do this.` });
            ...
            if (!equipments) return sendMessage({ client: client, message: message, content: `You don't have any equipment, ${master}.` });
            const equipment = equipments.filter(i => i.equipment.name.toLowerCase() === equipmentName.toLowerCase());
            if (equipment.length < 1) return sendMessage({ client: client, message: message, content: `You don't have that equipment, ${master}.` });
            ...
            if (!foods) return sendMessage({ client: client, message: message, content: `You don't have any food to give, ${master}.` });
            const food = foods.filter(i => i.food.name.toLowerCase() === foodName.toLowerCase());
            if (food.length < 1) return sendMessage({ client: client, message: message, content: `You don't have that food, ${master}.` });
            ...
            let messageFile = new Discord.MessageAttachment(canvas.toBuffer());
            return sendMessage({ client: client, message: message, content: `**${shinx.nick}** ${reaction[0]}`, files: messageFile });
            ...
            return sendMessage({ client: client, message: message, content: `Successfully released Shinx and reset all it's values.` });
            ...
            let messageFile = new Discord.MessageAttachment(canvas.toBuffer());
            return sendMessage({ client: client, message: message, content: `**${shinx.nick}** ${conversation.quote}`, files: messageFile });
        };
