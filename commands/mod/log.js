exports.run = async (client, interaction) => {
  ...
  try {
    ...
    let oldChannel = await LogChannels.findOne({
    ...
    if (!channelArg && !disableBool) {
      if (oldChannel) {
        return sendMessage({
          client: client,
          interaction: interaction,
          content: `The current logging channel is <#${oldChannel.channel_id}>.`,
        });
      }
      return sendMessage({
        client: client,
    ...
    if (disableBool)
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `Disabled logging functionality in **${interaction.guild.name}**.`,
      });
    await LogChannels.upsert({
      server_id: interaction.guild.id,
      channel_id: newLogChannel.id,
    });
    return sendMessage({
      client: client,
      interaction: interaction,
      content: `Logging has been added to ${newLogChannel}.`,
    });
  } catch (e) {
    // Log error
