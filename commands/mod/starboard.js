exports.run = async (client, interaction) => {
  ...
  try {
    ...
    let oldChannel = await StarboardChannels.findOne({
      where: { server_id: interaction.guild.id },
    });
    let oldStarLimitDB = await StarboardLimits.findOne({
      where: { server_id: interaction.guild.id },
    });
    let starlimit = null;
    if (oldStarLimitDB) {
    ...
    if (starlimitArg) {
      ...
      await StarboardLimits.upsert({
        server_id: interaction.guild.id,
        star_limit: starlimit,
      });
    }

    if (!targetChannel && !disableBool)
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `That channel does not exist in this server.`,
      });
    ...
    if (disableBool)
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `Disabled starboard functionality.`,
      });

    await StarboardChannels.upsert({
      server_id: interaction.guild.id,
    ...
    return sendMessage({
      client: client,
      interaction: interaction,
      content: `${targetChannel} is now **${interaction.guild.name}**'s starboard. ${starlimit} stars are required for a message to appear there.`,
    });
  } catch (e) {
    // Log error
