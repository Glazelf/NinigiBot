exports.run = async (client, interaction) => {
  ...
  try {
    ...
    if (!interaction.member.permissions.has("KICK_MEMBERS") && !adminBool)
      return sendMessage({
        client: client,
    ...
    if (
      ...
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `You don't have a high enough role to kick ${user.username} (${user.id}).`,
      });
    if (!member.kickable)
      return sendMessage({
        client: client,
        interaction: interaction,
        content: kickFailString,
      });
    ...
    await user
      .send({
        content: `You've been kicked from **${interaction.guild.name}** for the following reason: \`${reason}\``,
      })
