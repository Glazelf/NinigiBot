exports.run = async (client, message, args = []) => {
    ...
    try {
        ...
        if (!subCommand) {
            if (oldChannel) {
                return sendMessage({ client: client, message: message, content: `The current VC text channel is <#${oldChannel.channel_id}>.` });
            };
        ...
        if (subCommand == "disable") return sendMessage({ client: client, message: message, content: `Disabled VC text channel functionality in **${message.guild.name}**.` });

        await VCTextChannels.upsert({ server_id: message.guild.id, channel_id: targetChannel.id });

        return sendMessage({ client: client, message: message, content: `${targetChannel} is now **${message.guild.name}**'s VC text channel.` });
