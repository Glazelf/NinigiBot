...
export default async (interaction, ephemeral) => {
    try {
        ...
        if (!slowmodeSupportedChannelTypes.includes(interaction.channel.type)) return sendMessage({ client: interaction.client, interaction: interaction, content: `This channel type doesn't support slowmode.` });

        let time = interaction.options.getInteger("time");
        await interaction.channel.setRateLimitPerUser(time);
        return sendMessage({ client: interaction.client, interaction: interaction, content: `Slowmode set to ${time} seconds.`, ephemeral: ephemeral });
