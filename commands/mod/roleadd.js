exports.run = async (client, interaction) => {
  ...
  try {
    ...
    if (!interaction.member.permissions.has("MANAGE_ROLES") && !adminBoolUser)
      return sendMessage({
        client: client,
    ...
    if (descriptionArg) {
      ...
      if (description.length > selectDescriptionCharacterLimit)
        return sendMessage({
          client: client,
          interaction: interaction,
          content: `Role description must be ${selectDescriptionCharacterLimit} characters or less.`,
        });
    }
    ...
    if (role.managed == true)
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `I can't manage the **${role.name}** role because it is being automatically managed by an integration.`,
      });
    if (
      ...
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `I can't manage the **${role.name}** role because it is above my highest role.`,
      });
    if (
      ...
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `You don't have a high enough role to make the **${role.name}** role selfassignable.`,
      });
    ...
    if (roleIDs.length > 0) {
      ...
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `${role} is no longer eligible to be selfassigned.`,
      });
    } else {
      if (description) {
        await EligibleRoles.upsert({
          role_id: role.id,
          name: role.name,
      ...
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `${role} is now eligible to be selfassigned.`,
      });
    }
  } catch (e) {
    // Log error
