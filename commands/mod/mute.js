exports.run = async (client, interaction) => {
  ...
  try {
    ...
    if (!interaction.member.permissions.has("MODERATE_MEMBERS") && !adminBool)
      return sendMessage({
        client: client,
    ...
    if (!member)
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `Please provide a user to mute.`,
      });
    ...
    let minutesMuted = Math.floor(
      displayMuteTime - hoursMuted * 60 - daysMuted * 60 * 24,
    );
    let daysMutedDisplay =
      daysMuted > 0 ? daysMuted + (daysMuted == 1 ? " day " : " days ") : "";
    let hoursMutedDisplay =
      hoursMuted > 0
        ? hoursMuted + (hoursMuted == 1 ? " hour " : " hours ")
        : "";
    let minutesMutedDisplay =
      minutesMuted > 0
        ? minutesMuted + (minutesMuted == 1 ? " minute " : " minutes ")
        : "";
    displayMuteTime =
      daysMutedDisplay + hoursMutedDisplay + minutesMutedDisplay;

    if (!member)
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `Please use a proper mention if you want to mute someone.`,
      });
    // Check permissions
    let userRole = interaction.member.roles.highest;
    let targetRole = member.roles.highest;
    if (targetRole.position >= userRole.position && !adminBool)
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `You don't have a high enough role to mute ${member.user.username} (${member.id}).`,
      });
    if (!member.moderatable)
      return sendMessage({
        client: client,
        interaction: interaction,
        content: `I don't have permissions to mute this user.`,
      });
    ...
    if (member.communicationDisabledUntil) {
      // Check if a timeout timestamp exists
      if (member.communicationDisabledUntil > Date.now()) {
        // Only attempt to unmute if said timestamp is in the future, if not we can just override it
        muteTime = null;
    ...
    try {
      ...
      if (e.toString().includes("Missing Permissions"))
        return sendMessage({
          client: client,
          interaction: interaction,
          content: `Failed to toggle timeout on ${user.username}. I probably lack permissions.`,
        });
      // Log error
