exports.run = async (client, interaction) => {
  ...
  try {
    ...
    if (DiscordJSVersion.includes("dev"))
      DiscordJSVersion = DiscordJSVersion.split("dev")[0] + "dev";
    let memoryUsage = `${
    ...
    try {
      githubRepoResponse = await axios.get(
        `https://api.github.com/repos/${githubURLVars}`,
      );
      githubMasterResponse = await axios.get(
        `https://api.github.com/repos/${githubURLVars}/branches/master`,
      );
    } catch (e) {
      // console.log(e);
    ...
    let lastCommitTimestamp = Math.floor(
      new Date(githubMasterResponse.data.commit.commit.author.date).getTime() /
        1000,
    );

    let lastCommitMessage = `"[${
      githubMasterResponse.data.commit.commit.message.split("\n")[0]
    }](https://github.com/${githubURLVars}/commit/${
      githubMasterResponse.data.commit.sha
    })"`;
    ...
    if (client.shard)
      botEmbed.addField("Shards:", ShardUtil.count.toString(), true);
    botEmbed
    ...
    if (ownerBool)
      botEmbed.addField("Online Since:", `<t:${onlineSince}:R>`, true);
    if (githubRepoResponse)
      botEmbed.addField(
        "Github Stars:",
        `[${githubRepoResponse.data.stargazers_count}](https://github.com/${githubURLVars}/stargazers)⭐`,
        true,
      );
    if (githubMasterResponse)
      botEmbed.addField("Latest Commit:", lastCommitString, true);
    ...
    let botButtons = new Discord.MessageActionRow()
      .addComponents(
        new Discord.MessageButton({
          label: "Invite Bot",
          style: "LINK",
          url: `https://discordapp.com/oauth2/authorize?client_id=${client.user.id}&permissions=8&scope=bot%20applications.commands`,
        }),
      )
      .addComponents(
        new Discord.MessageButton({
          label: "App Directory",
          style: "LINK",
          url: `https://discord.com/application-directory/${client.user.id}`,
        }),
      )
      .addComponents(
        new Discord.MessageButton({
          label: "Github",
          style: "LINK",
          url: `https://github.com/${githubURLVars}`,
        }),
      )
      .addComponents(
        new Discord.MessageButton({
          ...
          style: "LINK",
          url: `https://discord.gg/${globalVars.ShinxServerInvite}`,
        }),
      );
    return sendMessage({
      client: client,
