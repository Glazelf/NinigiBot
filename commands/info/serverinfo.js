exports.run = async (client, interaction) => {
  ...
  try {
    ...
    if (guild.members.me.permissions.has("USE_EXTERNAL_EMOJIS") || adminBot)
      boosterString = boosterString + nitroEmote;
    // Icon and banner
    let icon = guild.iconURL(globalVars.displayAvatarSettings);
    let banner = null;
    if (guild.bannerURL())
      banner = guild.bannerURL(globalVars.displayAvatarSettings);
    // Rules
    let rules = null;
    ...
    if (guild.vanityURLCode)
      serverLinks += `discord.gg/[${guild.vanityURLCode}](https://discord.gg/${guild.vanityURLCode})\n`;
    ...
    if (
      guild.rulesChannel &&
      (interaction.member.permissions.has("VIEW_GUILD_INSIGHTS") || adminBool)
    )
      serverButtons.addComponents(
        new Discord.MessageButton({
          label: "Insights",
    ...
    let assetString = "";
    if (unmanagedEmoteCount > 0)
      assetString += `\nEmotes: ${unmanagedEmoteCount}/${emoteMax} 😳`;
    if (managedEmotes.size > 0)
    ...
    if (guild.features.includes("COMMUNITY") && guild.preferredLocale) {
      if (languages[guild.preferredLocale])
        serverEmbed.addField(
          "Language:",
          languages[guild.preferredLocale],
          true,
        );
    }
    serverEmbed
    ...
    if (client.shard)
      serverEmbed.addField("Shard:", `${shardNumber}/${ShardUtil.count}`, true);
    serverEmbed
      .addField(
        "Created:",
        `<t:${Math.floor(guild.createdAt.valueOf() / 1000)}:f>`,
        true,
      )
