exports.run = async (client, interaction) => {
  ...
  try {
    ...
    let memberCount = guildMembers.filter((member) =>
      member.roles.cache.find((loopRole) => loopRole == role),
    ).size;
    // Properties
    let roleProperties = "";
    if (role.hoist) roleProperties = `${roleProperties}Sorted seperately\n`;
    if (role.mentionable)
      roleProperties = `${roleProperties}Can be mentioned\n`;
    if (role.managed)
      roleProperties = `${roleProperties}Managed by integration\n`;
    ...
    if (permissionString.length > 1024)
      permissionString = `${permissionString.substring(0, 1021)}...`;
    // Embed
    ...
    if (role.hexColor !== defaultColor)
      roleEmbed.addField("Color:", role.hexColor, true);
    roleEmbed
